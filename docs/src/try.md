  ...
  <div class="sim-engine">
    ...
    <p class="sim-custom-status" id="simCustomStatus" role="status" aria-live="polite" aria-atomic="true"></p>
  </div>

  <div class="sim-settings-upload" id="settingsDropZone">
    <div class="drop-zone-content">
      <svg class="icon-upload" aria-hidden="true" focusable="false" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="drop-zone-sub" id="settingsUploadHelp">Files are parsed locally and never uploaded. Choose files or drop them here.</div>
    </div>
    <input class="settings-file-input" type="file" id="settingsFileInput" multiple accept=".json,application/json" aria-label="Choose settings JSON files" aria-describedby="settingsUploadHelp settingsUploadStatus settingsUploadError" />
  </div>
  <p class="settings-upload-status" id="settingsUploadStatus" role="status" aria-live="polite" aria-atomic="true"></p>
  <p class="settings-upload-error" id="settingsUploadError" role="alert" hidden></p>

  <div class="active-settings-panel" id="activeSettingsPanel" hidden>
    <div class="panel-header">
      <span class="panel-title">Active Settings Rules</span>
      <button class="btn btn-ghost btn-xs" id="clearSettingsBtn" type="button">Clear</button>
    </div>
  ...
  <figure class="lifecycle" aria-labelledby="sim-pipeline-label">
    <div class="lc-bar">
      <span class="lights"><i></i><i></i><i></i></span>
      <span class="lc-label" id="sim-pipeline-label">gate pipeline · stepping through</span>
    </div>
