...
Intelligent tool permission gate using tree-sitter AST parsing. Handles Bash commands, Read/Write/Edit/MultiEdit file operations, Glob/Grep searches, and MCP tools. Auto-allows known safe operations, asks for writes and unknown commands, blocks dangerous patterns.

**Claude Code:** Use as PreToolUse + PermissionRequest + PostToolUse hooks (native integration)
**Gemini CLI:** Use as BeforeTool + AfterTool hooks (native integration)
...
├── file_guards.rs       # Symlink guard for AI config files (Read/Write/Edit/MultiEdit)
...
1. **Input**: JSON from Claude Code's PreToolUse hook (includes `tool_name`, `cwd`, `permission_mode`, `tool_use_id`)
