...
**Gemini CLI (deprecated compatibility):** Use as a single BeforeTool hook for existing setups (requires v0.36.0+ for `ask` decision support). Google's consumer Gemini CLI sunset date was 2026-06-18; use Antigravity for new setups.
...
| Hook | Purpose | When it runs |
|------|---------|--------------|
| **PreToolUse** | Route all tool types, deny/ask dangerous operations (a hook allow is inert on agy, so tool-gates emits nothing for it; agy keeps the strictest decision), file guards, secret scanning. The whole gate runs here | Before tool execution |
...
- Antigravity: flat `{decision, reason}` where `decision` is `allow`/`ask`/`deny`/`force_ask` (the hard-ask floor uses `force_ask`). Antigravity resolves a tool call as the STRICTEST of the hook and native decisions (rank: allow=1 floor, ask=2, deny_unless_prior_grant=3, force_ask=4, deny=5), so the hook can only tighten: a hook `allow` (rank 1) never suppresses a prompt agy's own rules would show, so tool-gates emits nothing for it. Both `Allow` and "no opinion" produce empty stdout, so Antigravity's own fine-grained permission engine decides; tool-gates never speaks for an unrecognized command. (`decision` is required by the schema; emitting none relies on the currently-undocumented behavior that Antigravity then defers to its own engine.) The Pre output has no `additionalContext` field, so hints/Tier-3 are dropped and deny remediation folds into the reason. Prompt-free safe commands come from agy's native `permissions.allow` in `~/.gemini/antigravity-cli/settings.json` (a different file from the hooks path), generated by `tool-gates agy allowlist`.
...
- Flat `{decision, reason}` output for tightening decisions only. `Approve` (no opinion) and `Allow` both emit empty stdout: agy keeps the strictest of the hook and native decisions and `allow` is the lowest rank, so a hook allow is inert; tool-gates emits nothing and lets agy's engine decide. `Defer` collapses to `ask` (no resolver-suggestion path). Hard blocks emit `deny`; remediation context is folded into the reason.
...
- The hard-ask safety floor (pipe-to-shell, eval, dangerous substitution) maps to `force_ask`, not `ask`: Antigravity's plain `ask` honors a prior "Always Allow" grant, which would let a granted command bypass the floor, while `force_ask` always prompts. `permissionOverrides` is part of the schema but tool-gates does not emit it: a hook's `permissionOverrides` does not suppress the current call's prompt, so it buys nothing. MCP is not wired for Antigravity yet (its MCP tool-name format for hook matchers is undocumented).
...
├── apply_patch_parser.rs # Codex apply_patch diff-body parsing (Add/Update/Delete/Move paths)
...
├── design_lint.rs       # Opt-in UI-file content lint (generic/templated design patterns)
...
├── cache.rs             # Centralized ~/.cache/tool-gates directory paths
...
├── rules_schema.rs      # rules/*.toml deserialization types (shared with build.rs)
├── rules_export.rs      # rules export docs generator (gates, floor, hints, reminder/design catalogs)
...
`tool-gates rules export --format md --out docs/src` regenerates `docs/src/gates/*.md`, `docs/src/security-floor.md`, `docs/src/hints.md`, `docs/src/security-reminders.md`, and `docs/src/design-lint.md` from the rule, hint, and scanner catalogs, separate from `build.rs`'s `src/generated/` Rust output.
...
4. **Route by tool_name**:
   ...
   - **Read/Write/Edit** -> File guards (symlink detection for AI config files), then scratch auto-allow for write targets under a recognized scratch root: `$TOOL_GATES_SCRATCH` or the Claude session scratchpad (see Scratch Directory)
...
**Residual-expansion fail-closed.** The scratch fast-path only fires for a path tool-gates can resolve to a concrete string. After substituting the base token (only while the `TOOL_GATES_SCRATCH` env var is set: unset, the shell expands the token to "", so the gate leaves it literal instead of substituting its internal default), tracked variables (`is_under_scratch_with_vars`), the convention tokens `${PWD//\//-}` and `$CLAUDE_CODE_SESSION_ID`/`${CLAUDE_CODE_SESSION_ID:-..}` (resolved in `resolve_scratch_convention_tokens` from the gate's own env, which for a Bash command is the same env the shell expands against; the `PWD` slash-replacement is traversal-safe by construction), and `$HOME`/`$USER`/`~`, any path that still carries an unresolved expansion (`$X`, `${X}`, `${X:-..}`, `$(...)`, a positional/special param, or a backtick: `path_has_unresolved_expansion`) is **not** treated as scratch and falls through to a normal prompt. This closes the variable-indirection class where an untrackable `$VAR` that the shell expands to a `..` traversal would otherwise resolve under the base lexically and auto-allow. The canonical `$TOOL_GATES_SCRATCH/${PWD//\//-}/$CLAUDE_CODE_SESSION_ID/...` path (and the `:-default` fallback form) stays friction-free because its tokens resolve; an untracked variable in any position prompts.

**Claude Code native session scratchpad (second recognized root).** Claude Code creates a per-session scratchpad at `<tmpdir>/claude-<uid>/<project-segment>/<session-id>/scratchpad` at session start and injects the literal path into the agent's system prompt. Claude Code's own auto-allow for that directory covers only the file-tool read/write resolvers, not Bash, so redirects/`mkdir`/`tee`/`cp` into it would prompt. `is_under_scratch` therefore also matches this root (`is_claude_session_scratchpad` in `router.rs`), which every scratch call site inherits. Match rules: the temp base is `std::env::temp_dir()` canonicalized; the `claude-<uid>` component is exact (`libc::getuid()`; unix-only today, since the matcher's `/`-separator handling never fires against Windows backslash paths, so the unsuffixed-`claude` Windows arm is a fail-closed placeholder); the `<project-segment>` encodes the session's **launch** directory (not the hook's `cwd`; worktrees differ) and its over-length form carries an irreproducible hash suffix, so it is matched structurally as one `[A-Za-z0-9-]` component; the `<session-id>` component must equal the gate's own `CLAUDE_CODE_SESSION_ID` and be UUID-shaped (`is_uuid_shaped`). Scope ends at `scratchpad/`: the `tasks/` sibling and the session dir itself still prompt. Other clients never set the session env var, so this root is inert for them; every failure mode falls through to a normal prompt.
...
1. Create `rules/newgate.toml` (`priority` orders TOML files for code generation only, lower first; runtime dispatch order is the hand-maintained `GATES` array in `src/gates/mod.rs`, with `basics` last):
...
| `tool-gates agy allowlist [--apply]` | Generate the agy native `permissions.allow` block (`command(<prog>)` per unconditionally-safe program) for `~/.gemini/antigravity-cli/settings.json`. Prints by default; `--apply` merges (preserves existing entries, backs up). The native allowlist is what stops agy prompts for read-only commands, since a hook allow is inert |
...
| `tool-gates rules export --format md [--out PATH] [--rules-dir PATH]` | Regenerate gate, security-floor, hints, security-reminders, and design-lint docs (default `docs/src`) |
...
design_lint = false          # Opt-in UI-file design lint on Write/Edit (default: false)
```
